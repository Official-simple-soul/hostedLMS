  ...
  return (
    ...
    <div className="bg-white rounded-lg px-8 py-4 mt-6">
      <div className="flex space-x-12">
        <h1 className={`${profileOption==='Personal details'?"border-b border-black":"text-blue-ribbon-500"} cursor-pointer`} onClick={(e)=>setProfileOption(e.currentTarget.innerText)}>Personal details</h1>
        <h1 className={`${profileOption==='Professional details'?"border-b border-black":"text-blue-ribbon-500"} cursor-pointer`} onClick={(e)=>setProfileOption(e.currentTarget.innerText)}>Professional details</h1>
      </div>
      {
        profileOption === 'Personal details'?
        <>
      <div className="md:flex items-center md:space-x-12 mt-8">
        <div className="img relative cursor-pointer" onClick={()=>setUploadOption(!uploadOption)}>
          ...
          <div className={`${uploadOption?'block':'hidden'} upload absolute top-0 -right-10 shadow-lg bg-white p-4 rounded-lg`}>
            <div className="file cursor-pointer">
              <label htmlFor="img"><i class="fa-solid fa-image mr-1"></i>Upload Picture</label>
              <input type="file" id='img' className='hidden'/>
            </div>
            <h1 className='text-[red] mt-4 cursor-pointer' onClick={()=>setProfilePic(null)}>Delete Picture</h1>
          </div>
        </div>
